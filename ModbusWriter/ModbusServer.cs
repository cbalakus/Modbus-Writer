...
namespace ModbusWriter
{
    ...
    internal class TCPHandler
    {
        ...
        public delegate void DebutOutput(byte[] values, string type);
        public event DebutOutput _debugHandler = null;
        ...
        private void AcceptTcpClientCallback(IAsyncResult asyncResult)
        {
            ...
            try
            {
                ...
                _debugHandler?.Invoke(new byte[0], tcpClient.Client.RemoteEndPoint.ToString() + " Connected.");
                if (ipAddress != null)
                {
        ...
        private void ReadCallback(IAsyncResult asyncResult)
        {
            ...
            if (client != null)
            {
                //try
                //{
                //    _debugHandler?.Invoke(new byte[0], client.TcpClient.Client.RemoteEndPoint.ToString() + " Disconnected.");
                //}
                //catch
                //{
                ...
                {
                    networkStream.BeginRead(client.Buffer, 0, client.Buffer.Length, ReadCallback, client);
                    _debugHandler?.Invoke(client.Buffer, "RX");
                }
                catch (Exception)
                {
                }
    ...
    public class ModbusServer
    {
        ...
        public delegate void DebugOutput(byte[] bytes, string type);
        public event DebugOutput _output;
        #endregion
        ...
        private void ListenerThread()
        {
            if (!udpFlag & !serialFlag)
            {
                ...
                tcpHandler._debugHandler += (byte[] values, string type) =>
                {
                    _output?.Invoke(values, type);
                };
            }
            else if (serialFlag)
            {
        ...
        private void ReadCoils(ModbusProtocol receiveData, ModbusProtocol sendData, NetworkStream stream, int portIn, IPAddress ipAddressIn)
        {
            ...
            if (true)
            {
                ...
                try
                {
                    ...
                    _output?.Invoke(data.Take(data.Length).ToArray(), "TX");
                }
                catch (Exception) { }
            }
        }
